...
void main() {
  testWidgets('DataTable2 renders with DataRow.index()', (WidgetTester tester) async {
  ...
  testWidgets('DataTable2 applies placeholder text style', (WidgetTester tester) async {
    await tester.pumpWidget(MaterialApp(
        home: Material(
      child: buildTable(rows: [
        DataRow2.byIndex(
    ...
    var t = find.ancestor(of: find.text('A1'), matching: find.byType(DefaultTextStyle)).evaluate().first.widget as DefaultTextStyle;

    expect(t.style.color!.a, 0.6);
  });
  testWidgets('DataTable2, placholder text is 0.6 opacity', (WidgetTester tester) async {
    await tester.pumpWidget(MaterialApp(
        home: Material(
      child: buildTable(rows: [
        DataRow2.byIndex(
  ...
  testWidgets('DataTable2, divider thickness 0 shows no border', (WidgetTester tester) async {
    await tester.pumpWidget(MaterialApp(
        home: Material(
    ...
    expect(find.byType(Table).evaluate().where((e) => (e.widget as Table).border != null).length, 0);
  });

  testWidgets('DataRow2.clone() creates a copy with the same properties', (WidgetTester tester) async {
  ...
  testWidgets('DataRow2.clone() creates a copy with the overriden properties', (WidgetTester tester) async {
    const originalKey = ValueKey('original');
    const cellText = Text('Cell');
    final originalRow = DataRow2(
      key: originalKey,
  ...
  testWidgets('DataTable2, showing sort arrow when changing sort column', (WidgetTester tester) async {
    var sortCol = 1;
    var asc = false;
    ...
    expect((find.byType(Opacity).evaluate().first.widget as Opacity).opacity, 0);
    ...
    expect((find.byType(Opacity).evaluate().first.widget as Opacity).opacity, 1);
